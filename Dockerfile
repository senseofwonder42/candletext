...
# and `pip install candletext` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "candletext", "/bin/bash", "-c"]

# build package
RUN tox -e build
RUN mamba env export -n candletext -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "candletext", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "candletext", "YOUR_CONSOLE_SCRIPT"]
